const ICONS = {
        ...
        function loadData() {
            ...
            if (saved) {
                ...
                // Миграция: если в старых данных нет settingsUpdatedAt,
                // ставим текущее время, чтобы при первой синхронизации
                // локальные настройки уехали на сервер
                if (!appData.settingsUpdatedAt) {
                    appData.settingsUpdatedAt = Date.now();
        ...
        async function deleteOperation(id) {
            const ok = await confirmDialog('Удалить операцию?', 'Удаление', 'Удалить');
            if (!ok) return;
        ...
        // Асинхронный confirm через модалку — работает на всех мобильных,
        // в отличие от нативного confirm() который iOS Safari может блокировать
        let _confirmResolver = null;
        function confirmDialog(text, title = 'Подтверждение', okText = 'Да') {
        ...
        function confirmModalRespond(result) {
        ...
        async function clearAllData() {
            const ok = await confirmDialog(
                'Все операции, категории, шаблоны и начальные остатки будут удалены. Это действие нельзя отменить.',
                'Удалить ВСЕ данные?',
                'Удалить всё'
            );
            if (!ok) return;

            // Сохраняем реквизиты подключения — их не трогаем
            const keepSheetId = appData.sheetId;
            const keepDeployUrl = appData.deployUrl;

            // Чтобы удалить все операции ещё и с других устройств — заносим их id в deletedIds
            const allIds = appData.operations.map(op => op.id);
            const prevDeleted = appData.deletedIds || [];
            ...
            appData = {
                ...
                operations: [],
                sheetId: keepSheetId,
                deployUrl: keepDeployUrl,
                deletedIds: Array.from(new Set([...prevDeleted, ...allIds])),
                exchangeRate: DEFAULT_EXCHANGE_RATE,
                templates: [],
                cashes: [
                    { id: 'cash_nalichnye', name: '💵 Наличные', type: 'cash' },
            ...
            // Сразу отправляем на сервер, чтобы очистилось везде
            if (isSynced && isOnline) syncWithGoogleAppsScript();
        }
        ...
        async function deleteEditingOperation() {
            if (!editingOperationId) return;
            const idToDelete = editingOperationId;
            const ok = await confirmDialog('Удалить операцию?', 'Удаление', 'Удалить');
            if (!ok) return;
            appData.operations = appData.operations.filter(op => op.id !== idToDelete);
            if (!appData.deletedIds) appData.deletedIds = [];
            if (!appData.deletedIds.includes(idToDelete)) {
                appData.deletedIds.push(idToDelete);
            }
            saveData(true);
            updateDashboard();
            updateHistoryFilter();
        ...
        async function deleteTemplate(index) {
            const ok = await confirmDialog('Удалить шаблон?', 'Удаление', 'Удалить');
            if (!ok) return;
